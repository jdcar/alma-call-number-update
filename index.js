...
csv.parseStream(stream, { headers: true, delimiter: '\t' })
    .on("data", function (data) {
        ...
        const sendGetRequest = async () => {
            try {
                const res = await axios.get(`https://api-na.hosted.exlibrisgroup.com/almaws/v1/bibs/${data.MMSID}/holdings/${data.HoldingId}?apikey=${sandboxKey}`);
                ...
                const sendPutRequest = async () => {
                    try {
                        const resp = await axios.put(`https://api-na.hosted.exlibrisgroup.com/almaws/v1/bibs/${data.MMSID}/holdings/${data.HoldingId}?apikey=${sandboxKey}`, xml, headerConfig);
